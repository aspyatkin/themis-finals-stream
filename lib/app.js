...
function fetchRecentEvents (request, response, callback) {
  ...
  if (lastEventId != null) {
    ...
    } else if (request.identity === 'team') {
      query = query.where('team', true)
    } else if (request.identity === 'external') {
      query = query.where('external', true)
    }
...
app.get('/stream', getRemoteAddr, getIdentity, getLastEventId, (request, response) => {
  ...
  fetchRecentEvents(request, response, (err) => {
    ...
    let channel = `${namespace}:${request.identity}`
